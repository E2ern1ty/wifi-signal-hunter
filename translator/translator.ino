...
static String httpsPostJson(const char *path, const String &body,
  ...
  *httpCode = statusLine.startsWith("HTTP/") ? statusLine.substring(9, 12).toInt() : 0;
  // 跳过头
  ...
  String resp;
  while (client.connected() && client.available()) resp += (char)client.read();
  return resp;
}
...
static String httpsPostWav(const char *path, const int16_t *pcm, size_t samples,
  ...
  u32 = 16; memcpy(hdr + 12, &u32, 4);
  uint16_t u16 = 1; memcpy(hdr + 20, &u16, 2);        // PCM
  ...
  while (sent < dataLen) {
    ...
    if (client.write(p + sent, n) != n) return "";
    sent += n;
  }
  client.print(post);
...
static bool httpsPostBinary(const char *path, const String &body,
  ...
  client.setInsecure();
  client.setTimeout(20000);
  if (!client.connect(API_HOST, 443)) return false;
  client.printf("POST %s HTTP/1.1\r\n"
                "Host: %s\r\n"
                "Authorization: Bearer %s\r\n"
                "Content-Type: application/json\r\n"
...
static bool doAsr() {
  int code = 0;
  String resp = httpsPostWav(ASR_PATH, recPcm, recSamples, &code);
  if (resp.isEmpty()) { errMsg = "ASR: connect fail"; return false; }
